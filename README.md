...
Представляет собой макет интернет-магазина с возможностью выбора товара. Моей целью при работе над этим заданием было освоение документации, а именно - создание архитектуры и описание классов в проекте, а также изучение основ TypeScript. [Ссылка на макет](https://www.figma.com/design/0KMR7HxUUMyRPMjuCvQEN1/8-%D1%81%D0%BF%D1%80%D0%B8%D0%BD%D1%82.-%D0%92%D0%B5%D0%B1-%D0%BB%D0%B0%D1%80%D1%91%D0%BA?node-id=0-1&t=jEAiXeuAYj5TWOEV-0)
...
* Верстка страницы
* Нажатием на карточку товара открывается модальное окно с детальной информацией о товаре
* Нажатием на иконку корзины открывается корзина
* Возможность внесения детальной информации о товаре
* Нажатием на кнопку «Купить» товар добавляется в корзину, если не был добавлен в корзину раньше
* Нажатием на кнопку «Убрать» товар удаляется из корзины
* Возможность выбора способа оплаты
* Возможность ввода адреса доставки
* Валидация форм
* Возможность ввода почты и телефона покупателя
* При нажатии на кнопку оплаты появляется сообщение об успешной оплате и товары удаляются из корзины
* Закрытие модальных окон по клику вне модального окна и по клику на иконку «Закрыть» (крестик)
...
### 1. Класс Model
Абстрактный класс предназначен для расширения другого класса путем добавления методов и переменных другого класса к классу, который расширяется.

Класс имеет такие методы:
1. ```emitChanges(string, object)``` - создает событие об изменении модели.

Данный класс относится к прикладному слою архитектуры.

### 2. Класс Api
Класс предназначен для реализации сохранения данных пользователя в БД и получения данных для реализации каталога и корзины.
...
Класс имеет такие методы:
1. ```handleResponse(Response): void``` - проверка корректности работы сервера
2. ```get(string): void``` - получение данных с сервера
3. ```post(string, object, ApiPostMethods): void``` - добавление данными сервер

Данный класс относится к слою портов и адаптеров архитектуры.
...
3. ```emit(string, T): void``` - уведомление о наступлении события
...
Также у класса есть интерфейс ```IEvents``` содержащий методы класса. Данный класс относится к прикладному слою архитектуры.
...
2. ```setText(HTMLElement, unknown)``` - устанавливает текст
3. ```setDisabled(HTMLElement, boolean)``` - блокирует кнопку
4. ```setHidden(HTMLElement)``` - прячет элемент 
5. ```setVisible(HTMLElement)``` - показывает скрытый элемент
6. ```setImage(HTMLImageElement, string, string)``` - устанавливает картинку
...
Также у класса есть интерфейс ```IAuctionAPI``` содержащий методы класса. Обращение к серверу происходит через событие. Данный класс относится к прикладному слою архитектуры.

### 2. Класс AppState
Класс предназначен для заполнения корзины продуктами, а также удаления продукта из корзины, подсчета финальной суммы товаров и очистки корзины после оформления. Также данный класс должен содержать несколько продуктов, а именно массив класса Product.
...
Класс имеет такие методы:
1. ```setCatalog(IProductItem[])```- устанавливает каталог
2. ```setPreview(Product)``` - устанавливает предварительный просмотр
...
Также у класса есть интерфейс ```IAppState``` содержащий элементы класса. Установка каталога на страницу происходит через событие. Данный класс относится к прикладному слою архитектуры.

### 3. Класс Basket
Класс предназначен для отображения корзины на странице.
...
2. ```set selected(number)``` - блокировка кнопки
3. ```set total(number)``` - установка общей суммы
4. ```get button(): HTMLButtonElement``` - получение кнопки

Также у класса есть интерфейс ```IBasketView``` содержащий элементы класса. Переход на форму заказа происходит через событие. Данный класс относится к доменному слою архитектуры.

### 4. Класс BasketCard
Класс предназначен для реализации карточки в корзине и удаления продукта из корзины.
...
Удаление карточки происходит через события. Данный класс относится к прикладному слою архитектуры.
...
6. ```_button: HTMLButtonElement``` - кнопка добавления в корзину
...
Также у класса есть интерфейс ```ICard<T>``` содержащий элементы класса. Добавление карточки в корзину происходит через событие. Данный класс относится к прикладному слою архитектуры.
...
Также у класса есть интерфейс ```IContactsForm``` содержащий элементы класса. Проверка на корректность данных происходит через события. Данный класс относится к доменному слою архитектуры.

### 7. Класс Form<T>
Класс предназначен для отображения формы на странице, реализации кнопки отправки, установки значений.

Конструктор принимает такие аргументы:
1. ```_submit: HTMLButtonElement``` - кнопка отправки
2. ```_errors: HTMLElement``` - сообщение об ошибке

Класс имеет такие методы:
1. ```onInputChange(keyof T, string)``` - изменение полей формы
2. ```set valid(boolean)```, ```set errors(string)``` - установка значений
3. ```render(Partial<T> & IFormState)``` - обновление формы

Также у класса есть интерфейс ```IFormState``` содержащий элементы класса. Изменение полей, отправка данных или открытие новой формы происходят через события. Данный класс относится к прикладному слою архитектуры.

### 8. Класс Modal
Класс предназначен для реализации модальных окон на странице.

Конструктор принимает такие аргументы:
1. ```_closeButton: HTMLButtonElement``` - кнопка закрытия
...
Также у класса есть интерфейс ```IModalData``` содержащий элементы класса. Закрытие и открытие окна происходит через события. Данный класс относится к прикладному слою архитектуры.

### 9. Класс Order
Класс предназначен для реализации формы заказа, а также отображения на странице.
...
Также у класса есть интерфейс ```IOrderForm``` содержащий элементы класса. Проверка на корректность данных происходит через события. Данный класс относится к доменному слою архитектуры.

### 10. Класс Page
Класс предназначен для реализации главной страницы, подсчета количества товаров в корзине, открытия корзины, установки каталога на страницу, блокировки прокрутки страницы при открытии модального окна.
...
Также у класса есть интерфейс ```IPage``` содержащий элементы класса. Открытие корзины происходит через события. Данный класс относится к прикладному слою архитектуры.

### 11. Класс Product
Класс предназначен для описания характеристик продукта.
...
7. ```isAddBasket: boolean``` - была ли нажата кнопка добавления продукта в корзину
...
Также у класса есть интерфейс ```IProductItem``` содержащий элементы класса. Данный класс относится к доменному слою архитектуры.
...
Также у класса есть интерфейс ```ISuccess``` содержащий элементы класса. Данный класс относится к доменному слою архитектуры.
